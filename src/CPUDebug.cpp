...
uint16_t getImm16(uint8_t byte1, uint8_t byte2)
{
    return (byte2 << 8) + byte1;
}

void printOpcode(uint8_t opcode, uint8_t byte1, uint8_t byte2, uint8_t byte3)
{
    if (opcode == 0xCB)
    {
        // Prefix
    ...
    {
        // ld r16, imm16
        std::cout << std::format("ld {}, {}", getR16Name((opcode & 0b00110000) >> 4), getImm16(byte1, byte2));
    }
    ...
    {
        // ld [imm16], sp
        std::cout << std::format("ld [{}], sp", getImm16(byte1, byte2));
    }
    ...
    {
        // ld r8, imm8
        std::cout << std::format("ld {}, {}", getR8Name((opcode & 0b00111000) >> 3), byte1);
    }
    ...
    {
        // jr imm8
        std::cout << std::format("jr {}", byte1);
    }
    else if ((opcode & 0b11100111) == 0b00100000)
    {
        // jr cond, imm8
        std::cout << std::format("jr {}, {}", getCondName((opcode & 0b00011000) >> 3), byte1);
    }
    ...
    {
        // add a, imm8
        std::cout << std::format("add a, {}", byte1);
    }
    else if (opcode == 0b11001110)
    {
        // adc a, imm8
        std::cout << std::format("adc a, {}", byte1);
    }
    else if (opcode == 0b11010110)
    {
        // sub a, imm8
        std::cout << std::format("sub a, {}", byte1);
    }
    else if (opcode == 0b11011110)
    {
        // sbc a, imm8
        std::cout << std::format("sbc a, {}", byte1);
    }
    else if (opcode == 0b11100110)
    {
        // and a, imm8
        std::cout << std::format("and a, {}", byte1);
    }
    else if (opcode == 0b11101110)
    {
        // xor a, imm8
        std::cout << std::format("xor a, {}", byte1);
    }
    else if (opcode == 0b11110110)
    {
        // or a, imm8
        std::cout << std::format("or a, {}", byte1);
    }
    else if (opcode == 0b11111110)
    {
        // cp a, imm8
        std::cout << std::format("cp a, {}", byte1);
    }
    ...
    {
        // jp cond, imm16
        std::cout << std::format("jp {}, {}", getCondName((opcode & 0b00011000) >> 3), getImm16(byte1, byte2));
    }
    else if (opcode == 0b11000011)
    {
        // jp imm16
        std::cout << std::format("jp {}", getImm16(byte1, byte2));
    }
    ...
    {
        // call cond, imm16
        std::cout << std::format("call {}, {}", getCondName((opcode & 0b00011000) >> 3), getImm16(byte1, byte2));
    }
    else if (opcode == 0b11001101)
    {
        // call imm16
        std::cout << std::format("call {}", getImm16(byte1, byte2));
    }
    ...
    {
        // ldh [imm8], a
        std::cout << std::format("ldh [{}], a", byte1);
    }
    else if (opcode == 0b11101010)
    {
        // ld [imm16], a
        std::cout << std::format("ld [{}], a", getImm16(byte1, byte2));
    }
    ...
    {
        // add sp, imm8
        std::cout << std::format("add sp, []", byte1);
    }
    else if (opcode == 0b11111000)
    {
        // ld hl, sp + imm8
        std::cout << std::format("ld hl, sp + []", byte1);
    }
